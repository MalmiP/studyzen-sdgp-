...
export default function Dashboard() {
  ...
  const [activities, setActivities] = useState<Activity[]>([]); // Specify array type here
  const [quizCount, setQuizCount] = useState<number | null>(null); // Specify number or null type
  const [error, setError] = useState<string | null>(null);

  const fetchDashboardData = async () => {
    try {
      // Define the type of the response explicitly
      const [statsRes, activitiesRes, progressRes] = await Promise.all([
        axios.get<DashboardStats>('http://localhost:3001/api/dashboard/stats'),
        axios.get<Activity[]>('http://localhost:3001/api/dashboard/activities'),
        axios.get<ProgressData>('http://localhost:3001/api/dashboard/progress')
      ]);
